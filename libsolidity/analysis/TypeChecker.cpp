...
bool TypeChecker::visit(FunctionCall const& _functionCall)
{
	...
	{
		if (m_scope->sourceUnit().annotation().experimentalFeatures.count(ExperimentalFeature::V050))
			m_errorReporter.typeError(_functionCall.location(), "Event calls have to be prefixed by \"emit\".");
		else
			m_errorReporter.warning(_functionCall.location(), "Calling events without \"emit\" prefix is deprecated.");
	}
